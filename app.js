...
const MUSHIKUI_OPTIONS = [
  { value:'off',    label:'なし' },
  { value:'easy',   label:'優（1文字）' },
  { value:'normal', label:'普（2文字）' },
  { value:'hard',   label:'難（3文字〜）' }
];
...
function renderGradeCards() {
  ...
  container.innerHTML = '';
  Object.entries(GRADE_TEMPLATES).forEach(([key, g]) => {
...
function selectGrade(gradeKey) {
  ...
  document.querySelectorAll('.grade-card').forEach(c =>
    c.classList.toggle('selected', c.dataset.grade === gradeKey));

  const grade = GRADE_TEMPLATES[gradeKey];
  document.getElementById('topics-title').textContent = `${grade.icon} ${grade.label}のメニュー`;
  ...
  grade.topics.forEach(topic => {
    ...
    btn.innerHTML = `
      ...
      <div class="topic-desc">${esc(topic.desc)}</div>`;
    btn.addEventListener('click', () => applyTopic(topic, grade));
    list.appendChild(btn);
  });

  document.getElementById('topics-area').removeAttribute('hidden');
}

// ── トピック適用 → カスタムタブへ ──
function applyTopic(topic, grade) {
  daionList = [];
  daionCounter = 0;
  topic.daions.forEach(d => addDaion(
    d.operation, d.digits, d.count,
    d.writtenOps || [d.operation], d.mushikuiLevel || 'off'
  ));
  showToast(`${grade.label}「${topic.name}」を設定しました`);
  switchTab('custom');
}
...
function renderDaionList() {
  ...
  daionList.forEach((daion, idx) => {
    ...
    const mushikuiPills = MUSHIKUI_OPTIONS.map(opt => {
      ...
      const ac = isActive ? `active-${opt.value}` : '';
      return `<label class="check-pill mushikui-level-pill ${ac}" data-val="${opt.value}">
  ...
  container.querySelectorAll('.mushikui-radio').forEach(radio =>
    radio.addEventListener('change', e => {
      ...
      pills.forEach(pill => {
        ...
        if (inp && inp.checked) pill.classList.add('active-' + inp.value);
      });
    }));
